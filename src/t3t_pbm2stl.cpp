...
int main(int ac, char *av[])
{
    ...
    if (opts.unicode)
    { // overrides PBM specification
        opts.pbm_path = make_ASCII_Unicode_string(opts.unicode) + ".pbm";
    }

    if (!opts.pbm_path.empty())
    { // single PBM input specified
...
{
    if (unicode < 0x80)
    { // TODO: Special handling for special ASCII chars like space?
        char asciistring[3];
...
int parse_options(int ac, char *av[])
{
    ...
    try
    {
        ...
        desc.add_options()("help", "produce this help message")("unicode,u", bpo::value<std::string>(&unicode_arg), "specify input unicode (overrides other input args)")("ascii,a", bpo::value<std::string>(&opts.ASCII), "specify input ASCII character (overrides input PBM)")("pbm,p", bpo::value<std::string>(&opts.pbm_path), "specify input PBM path (overrides YAML)")("stl,s", bpo::value<std::string>(&opts.stl_path), "specify output STL path (only useful if input specified here)")("obj,o", bpo::value<std::string>(&opts.obj_path), "specify output OBJ path (only useful if input specified here)")("yaml,y", bpo::value<vector<string>>(&yaml_paths), "specify YAML configuration file(s)");
        bpo::variables_map vm;

        bpo::positional_options_description posopt;
        posopt.add("yaml", -1);
        bpo::store(bpo::command_line_parser(ac, av).options(desc).positional(posopt).run(), vm);
        bpo::notify(vm);
        ...
        if (opts.foot.mode == pyramids)
        {
            ...
            if (config["pyramid top column height"])
                get_yaml_dim_node(config, "pyramid top column height", opts.foot.pyramid_top_column_height);
            else
                opts.foot.pyramid_top_column_height.set(0, mm);
        ...
        float height_sum_mm = body_top_strip_mm + body_bottom_margin_mm + opts.depth_of_drive.as_mm() + opts.foot.Z.as_mm();
