...
static struct argp_option options[] = {
    ...
    {"unicode", 'u', "UNICODE", 0, "input character (hex unicode)"},
...
int main(int argc, char **argv)
{
  ...
  if (arguments->character == NULL || strlen(arguments->character) == 0) {
    if (arguments->unicode == 0) {
      fprintf(stderr, "No character oder unicode specified.\r\n");
      exit(1);
    }
  }
  ...
  if (arguments->character)
    ...
    error = FT_Load_Char(face, arguments->unicode, FT_LOAD_RENDER);
  ...
  for (y = 0; y < type_height; y++)
  {
    for (x = 0; x < type_width; x++)
    {
      *outbuf = 0x00;
      ...
      if ((x >= char_left) && (x < char_left + char_width) && (y >= char_top) && (y < char_top + char_height))
      {
        x2 = x - char_left;
        y2 = y - char_top;
                        ...
                        char *filename)
{
  ...
  for (y = 0; y < bitmap->height; y++)
  {
    for (x = 0; x < bitmap->width; x++)
    {
      ...
      fputc(c, fp);
      fputc(c, fp);
      fputc(c, fp); // TODO remove 3; just for visual scaling in Kate
    }
    fputc('\n', fp);
  }
...
static error_t parse_opt(int key, char *arg, struct argp_state *state)
{
  ...
  case 'c':
    ...
    break;
  case 'u':
    arguments->unicode = strtoul(arg, NULL, 16);
    break;
  case 'd':
